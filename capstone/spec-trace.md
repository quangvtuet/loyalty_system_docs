...
| I-5 Anti-tamper — Client sends forged tier in partner earn payload | `POST /partner-earn` with `{"tier":"PLATINUM"}` for a GOLD member and a SILVER member | Gateway strips the forged field; server calculates points using authoritative projected tier (GOLD 1.5x → 300 pts, SILVER 1.0x → 200 pts); projected tier unchanged | `NEG-I5-02` |
