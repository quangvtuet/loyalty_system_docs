...
public final class RedemptionEngineService {
    ...
    /**
     * G6-T01 service-level step: intake and reserve FIFO debit without dispatching fulfillment yet.
     * Transitions PENDING -> IN_PROGRESS.
     */
    public RedemptionOrder createAndReserveOrder(String memberId, String rewardItemId) {
    ...
    /**
     * G6-T02 service-level step: cancel an order that is in PENDING state.
     * Transitions PENDING -> CANCELLED.
     */
    public void cancelPendingOrder(RedemptionOrder order, String reason) {
        stateModule.toCancelled(order, reason);
    }

    /**
     * G6-T03 service-level step: partner confirms delivery for an IN_PROGRESS order.
     * Transitions IN_PROGRESS -> FULFILLED.
     */
    public void fulfillOrder(RedemptionOrder order) {
        stateModule.toFulfilled(order);
    }

    /**
     * G6-T04 service-level step: partner fails delivery for an IN_PROGRESS order.
     * Transitions IN_PROGRESS -> FAILED.
     */
    public void failOrder(RedemptionOrder order, String reason) {
        stateModule.toFailed(order, reason);
    }

    /**
     * G6-T05 service-level step: run CON.3 compensating action on a FAILED order.
     * Restores FIFO batches and transitions FAILED -> REVERSED.
     */
    public void reverseOrder(RedemptionOrder order) {
